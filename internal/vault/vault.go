	...
	case kubernetesAuth.ServiceAccountRef != nil:
		...
		tokenrequest, err := v.createToken(context.Background(), kubernetesAuth.ServiceAccountRef.Name, &authv1.TokenRequest{
			Spec: authv1.TokenRequestSpec{
				...
				// Providing additional audiences is not considered a security risk as if someone would create an Issuer in another namespace/globally with the same audiences
				// in attempt to highjack the certificate vault (if setup explicitly allow sa:namespace) won't authorise the conneciton
				// as token subject won't match vault backend role requirement to have SA originated from the specific namespace.
				Audiences: audiences,
